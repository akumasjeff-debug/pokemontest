...
function getThemeDataById(id) {
    var data = themePD.find(function(item) {
        return item.id === id;
    });
...
function html_01(dataId, divID) {
    ...
    for (var i = 0; i < content.length; i++) {
        ...
        // --- 核心修正：加入 pokemon-ball-fix 類別 ---
        html += '<div class="ball-title-wrap">';
        html += '<img class="pokemon-ball-fix" src="img/icon/icon-ball.svg" alt="icon-ball">';
        html += '<span class="pokemon-name-text">' + item.name + "</span>";
        html += '<img class="pokemon-ball-fix" src="img/icon/icon-ball.svg" alt="icon-ball">';
        html += "</div>";
        html += "</a>";
    }
...
----------------------*/
function html_PDslider(dataId, divID, customClass = "") {
    var content = getThemeDataById(dataId);
    var html = "";
    for (var i = 0; i < content.length; i++) {
        var item = content[i];
        // 將 max-w-1/6 移除，改由 Swiper 的 slidesPerView 自動控制寬度
        html += '<a href="' + item.link + '" target="' + item.target + '" title="' + item.alt + '" class="shadow1 swiper-slide pb-2 group transition-group text-center rounded-3xl overflow-hidden bg-white border hover:border-nintendoRed1 hover:shadow-md ' + customClass + '">';
        html += '<div class="absolute top-0 flex w-full justify-center pointer-events-none opacity-0 group-hover:opacity-100"><img src="img/icon/icon-item-search-y.svg" class="block"></div>';
        html += '<img src="' + item.path + item.imgL + '" class="block mb-2 w-full">';
        html += '<div class="flex items-center justify-center h-10 mx-3 mb-2 font-bold text-md sm:text-sm text-black border-b border-nintendoRed1">' + item.subject + "</div>";
        html += '<div class="w-auto h-10 mx-auto mb-1 line-5 text-sm">' + item.name + "</div>";
        // 修正了末端的 <div> 標籤屬性
        html += '<div class="inline-flex items-center justify-center px-0 h-8 text-black rounded-2xl border-nintendoGray4 border mx-auto mb-0 text-sm"><img class="-mt-2 px-l ball" src="img/icon/icon-ball-l.svg">' + item.description + '<img class="-mt-3 px-r" src="img/icon/icon-ball-r.svg"></div>';
        html += "</a>";
    }
    $(divID).html(html);
}
...
html_PDslider("PD04", "#section_PD04");
html_PDslider("PDv1", "#section_PDv1");
html_PDslider("PDv2", "#section_PDv2");

// 支援自定義數量的 Swiper 初始化
function initSwiper(containerClass, count = 6) {
    new Swiper("." + containerClass + " .swiper-container", {
        effect: "slide",
        loop: true,
        slidesPerView: count, // 使用傳入的數量
        spaceBetween: 26,
        speed: 1000,
        autoplay: { delay: 3000, disableOnInteraction: false },
        pagination: { el: "." + containerClass + " .swiper-pagination", clickable: true },
        navigation: {
            nextEl: "." + containerClass + " .swiper-button-next-small",
            prevEl: "." + containerClass + " .swiper-button-prev-small",
        },
        breakpoints: {
            ...
            768: { slidesPerView: Math.min(4, count), spaceBetween: 10 },
            1366: { slidesPerView: count, spaceBetween: 15 },
        },
    });
}

// 啟動
initSwiper("swiperPD01", 6);
initSwiper("swiperPD02", 6);
initSwiper("swiperPD03", 6);
initSwiper("swiperPD04", 3);
initSwiper("swiperPDv1", 6);
initSwiper("swiperPDv2", 6);





new Swiper(".swiperPD04 .swiper-container", {
    effect: "slide",
    loop: false,
    slidesPerView: 3,
    spaceBetween: 26,
    speed: 1000,
    autoplay: { delay: 3000, disableOnInteraction: false },
    // 這裡沒寫 pagination 和 navigation，所以不會出現圓點和箭頭
    breakpoints: {
        320: { slidesPerView: 2, spaceBetween: 8 },
